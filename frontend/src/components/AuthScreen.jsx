 ...
 * Muestra Google OAuth + email/password como fallback.
 *
 * Props:
 *   onAuth: () => void  ← se llama cuando el usuario se autentica
 */
...
export default function AuthScreen() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // La redirección la maneja Supabase — no hace falta hacer nada más
    } catch (e) {
      setError(e.message)
  ...
  const handleEmail = async (e) => {
    ...
    try {
      if (modo === 'login') {
        ...
        setMensaje('Revisá tu email para confirmar la cuenta.')
      }
    } catch (e) {
      setError(
  ...
  return (
    <div style={s.root}>
      <div style={s.card}>
        ...
        <div style={s.tagline}>
          Evaluá tu idea de negocio con IA en 60 segundos
        </div>
        ...
        <button
          ...
          style={{ ...s.btnGoogle, ...(cargandoGoogle ? s.btnDisabled : {}) }}
        >
          {cargandoGoogle ? (
            <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={s.spinner} /> Conectando con Google...
            </span>
          ) : (
        ...
        <form onSubmit={handleEmail} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <button
            ...
            style={{ ...s.btnEmail, ...(cargando || !email || !password ? s.btnDisabled : {}) }}
          >
            {cargando
              ? 'Cargando...'
        ...
        <div style={s.legal}>
          Al continuar aceptás los términos de uso. Tus ideas son tuyas.
        </div>
      </div>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0 }
        ...
        input:focus { outline: none; border-color: #7c4dff !important }
